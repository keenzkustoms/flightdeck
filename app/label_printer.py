...
class LabelPrinter:
    ...
    def render_spool_label(self, spool: dict) -> Image.Image:
        img = Image.new("RGB", (449, 331), "white")
        draw = ImageDraw.Draw(img)
        swatch = spool.get("color_hex") or "#888888"
        draw.rectangle((0, 0, 70, 331), fill=swatch)
        if _luminance(swatch) > 0.85:
            draw.rectangle((0, 0, 70, 330), outline="black")

        font_bold = _font("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 28)
        font_body = _font("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 21)
        font_small = _font("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 14)
        font_badge = _font("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 16)
        ...
        draw.text((x, 34), _ellipsize(draw, material, font_bold, 250), fill="black", font=font_bold)
        draw.text((x, 78), _ellipsize(draw, brand, font_body, 250), fill="black", font=font_body)
        draw.text((x, 112), _ellipsize(draw, color_name, font_body, 250), fill="black", font=font_body)
        draw.text((360, 36), f"#{spool.get('id', '-')}", fill="black", font=font_badge)
        ...
        bottom = f"{round(float(spool.get('label_weight_g') or 0))}g · {added}"
        draw.text((x, 280), bottom, fill="black", font=font_small)

        qr_url = f"https://flightdeck.tail7de73e.ts.net/#/spool/{spool.get('id')}"
        qr = _qr_image(qr_url)
        if qr:
            img.paste(qr.resize((82, 82)), (352, 224))
        else:
            draw.rectangle((352, 224, 434, 306), outline="black")
            draw.text((365, 254), "QR", fill="black", font=font_body)
        return img
    ...
    def print_spool_label(self, spool: dict) -> bool:
        ...
        try:
            ...
            instructions = convert(qlr=qlr, images=[image], label=self.LABEL_SIZE, rotate="90", threshold=70.0, dither=False)

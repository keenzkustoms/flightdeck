...
- Branch: main
- Latest commit: current HEAD after this handoff (`Shift Bambu skip thumbnail upward`)
...
Recent work:
- Bambu skip-object thumbnail layer now supports image-only X/Y offsets; current H2D trial keeps the red overlay locked and shifts the rotated thumbnail up by `-18%` to bring the star closer to the centre of `#439`.
...
- Current H2D trial values: `map_rotation=0`, `map_image_rotation=45`, `map_image_offset_x=0`, `map_image_offset_y=-18`.
